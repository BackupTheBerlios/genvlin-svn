...
/** The main class for plotting the data in an extra panel.
 * Its a very simple implemention of a plot class!!
 *
 * @author  Peter Karich
 */
public class GPlotPanel extends JPanel
    ...
    /** @return the standard coordinate bounds of the real coordinate
     *   and set the locale to default locale
     */
...
/*    public void addFunction()
    ...
    public void plotFunction(Graphics g)
    {
        ...
        for(int i=0; i<cB.dX(); i++)
        { a=yToWin(fkt(xToCoord(i)));
          //Log.log("a "+a+";  fkt(xToCoord(i)) "+fkt(xToCoord(i))        +";  xToCoord(i) "+xToCoord(i)+";  i "+i);
          xyPoint.draw(g, i, a);
        }
    }*/
     ...
     * data.add(new XYData(x, y, XYData.CROSS, dataColor[color], data.size()+""));
     *
     * colorDataIndex++;
     * }*/
    ...
    public void addData(VectorInterface xVector, VectorInterface yVector) {
        init(new XYData(xVector, yVector));
    }
    
    /** This method adds specified data to panel
     */
    public void addData(XYInterface xyVector) {
        init(new XYData(xyVector));
    }
    
    private void init(XYData xyData) {
        int color = colorDataIndex;
        automaticOneScale(colorDataIndex);
        color %= dataColor.length;
        xyData.setColor(dataColor[color]);
    ...
    /** This method scales the window so that we can see the specified data
     ** (in the most cases) very good.
     */
    protected void automaticScale(int index) {
        if(pool.size() ==0 || index >= pool.size() || index <0) return;
    ...
    /** This method scales the window so that we can see the last added data
     ** (in the most cases) very good.
     */
    ...
    /** This method sets the data in this plotpanel which should scaled
     * automatically for one time. -1 indicates that no scaling will happen.
     */
    ...
    /** The MouseListener provides distance measurement via dragging,
     * zooming via drag+shift and popupmenu displaying.
     */
    ...
    public JPopupMenu createPopup(MouseEvent me) {
        ...
        for(int i=0; i< pool.size(); i++) {
            JMenu menu = new JMenu("Data:"+i);
            menu.setForeground(getColor(i));
            
            mi = new JMenuItem("Automatic scaling:"+i);
            mi.addActionListener(this);
            menu.add(mi);
            
            mi = new JMenuItem("Remove:"+i);
            mi.addActionListener(this);
            menu.add(mi);
            popMenu.add(menu);
        }
    ...
    public void actionPerformed(ActionEvent ae) {
        ...
        int i = -1;
        try {
            i= Integer.parseInt(str.substring(str.indexOf(':')+1));} catch(NumberFormatException nfe) {
                return;}
    ...
    /*//incompatible return types "void JComponent.remove(int)"
    ...
    public void removeVectorListener(CollectionListener cl) {
        pool.removeVectorListener(cl);
    }
